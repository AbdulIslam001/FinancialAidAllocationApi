...
var session = db.Sessions.OrderByDescending(sess => sess.id).FirstOrDefault();
                ...
                if (amount != null)
                {
                    ...
                    foreach (var degree in degrees)
                    {
                        var semesters = db.Students.Where(s => s.degree == degree)
                                                   .Select(p => new { p.semester, p.section })
                                                   .Distinct();

                        foreach (var semSec in semesters)
                        {
                            var students = db.Students.Where(stu => stu.degree == degree &&
                                                                    stu.semester == semSec.semester &&
                                                                    stu.section == semSec.section)
                                                      .OrderByDescending(stu => stu.cgpa)
                                                      .ToList();

                            var count = students.Count();
                            Console.WriteLine("student count of "+semSec.section+count);
                            int totalScholarship = (count >= 40) ? 3 : (count >= 30 && count < 40) ? 2 : 1;

                            var topStudents = students.Take(totalScholarship)
                                                      .Select(stu => new
                                                      {
                                                          stu.student_id,
                                                          stu.cgpa
                            ...
                            {
                                var topStudent = topStudents[t];

                                var meritBase = new MeritBase
                                {
                                    studentId = topStudent.student_id,
                                    session = session.session1,
                                    position = t + 1
                                };
                                ...
                                var financialAid = new FinancialAid
                                {
                                    ...
                                    aidtype = "MeritBase",
                                    applicationId = topStudent.student_id,
                                    amount = (t == 0) ? first.ToString() : (t == 1) ? second.ToString() : third.ToString()
                                };
                    ...
                    var result = db.Students.Join(db.MeritBases,
                                                  s => s.student_id,
                                                  m => m.studentId,
                                                  (s, m) => new { s, m.position })
                                            .Distinct()
                                            .ToList();
